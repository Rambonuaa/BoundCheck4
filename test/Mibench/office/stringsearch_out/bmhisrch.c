...
__boundcheck_metadata_store((void *)(&s),(void *)((size_t)(&s)+sizeof(s)*8-1));
      ...
      for ( ;; )
      {
            ...
            j = patlen - 1;
            s = (char *)string + (i - j);
            while ( --j >= 0 && toupper(s[__boundcheck_ptr_cast_to_array_reference(94,43,"bmhi_search",(void *)(s),(void *)(s+j),j)]) == pat[__boundcheck_ptr_cast_to_array_reference(94,54,"bmhi_search",(void *)(pat),(void *)(pat+j),j)] )
                  ;
            if ( j < 0 )                                    /* rdg 10/93 */
                  return s;                                 /* rdg 10/93 */
