...
__boundcheck_metadata_store((void *)(&si),(void *)((size_t)(&si)+sizeof(si)*8-1));
  ...
  int lookbits;
__boundcheck_metadata_store((void *)(&lookbits),(void *)((size_t)(&lookbits)+sizeof(lookbits)*8-1));
int  ctr;
...
__boundcheck_metadata_store((void *)(&code),(void *)((size_t)(&code)+sizeof(code)*8-1));
  ...
  if (*(d_derived_tbl **)(__boundcheck_ptr_reference(139,8,"jpeg_make_d_derived_tbl",(void *)(pdtbl),(void *)(pdtbl))) == NULL)
    *(d_derived_tbl **)(__boundcheck_ptr_reference(140,6,"jpeg_make_d_derived_tbl",(void *)(pdtbl),(void *)(pdtbl))) = (d_derived_tbl *)
      (*(void *(*)(j_common_ptr, int, size_t))(__boundcheck_ptr_reference(141,21,"jpeg_make_d_derived_tbl",(void *)(cinfo->mem->alloc_small),(void *)cinfo->mem->alloc_small))) ((j_common_ptr) cinfo, JPOOL_IMAGE,
				  SIZEOF(d_derived_tbl));
  dtbl = *(d_derived_tbl **)(__boundcheck_ptr_reference(143,11,"jpeg_make_d_derived_tbl",(void *)(pdtbl),(void *)(pdtbl)));
  ...
  p = 0;
  for (l = 1; l <= 16; l++) {
    for (i = 1; i <= (int) htbl->bits[_RV_insert_check(0,17,151,28,"jpeg_make_d_derived_tbl",l)]; i++)
      huffsize[_RV_insert_check(0,257,152,7,"jpeg_make_d_derived_tbl",p++)] = (char) l;
  }
  huffsize[_RV_insert_check(0,257,154,3,"jpeg_make_d_derived_tbl",p)] = 0;
  
  /* Figure C.2: generate the codes themselves */
  /* Note that this is in code-length order. */
  
  code = 0;
  si = huffsize[_RV_insert_check(0,257,160,8,"jpeg_make_d_derived_tbl",0)];
  p = 0;
  while (huffsize[_RV_insert_check(0,257,162,10,"jpeg_make_d_derived_tbl",p)]) {
    while (((int) huffsize[_RV_insert_check(0,257,163,19,"jpeg_make_d_derived_tbl",p)]) == si) {
      huffcode[_RV_insert_check(0,257,164,7,"jpeg_make_d_derived_tbl",p++)] = code;
      code++;
    }
    code <<= 1;
    si++;
  ...
  p = 0;
  for (l = 1; l <= 16; l++) {
    if (htbl->bits[_RV_insert_check(0,17,175,9,"jpeg_make_d_derived_tbl",l)]) {
      dtbl->valptr[_RV_insert_check(0,17,176,7,"jpeg_make_d_derived_tbl",l)] = p; /* huffval[] index of 1st symbol of code length l */
      dtbl->mincode[_RV_insert_check(0,17,177,7,"jpeg_make_d_derived_tbl",l)] = huffcode[_RV_insert_check(0,257,177,26,"jpeg_make_d_derived_tbl",p)]; /* minimum code of length l */
      p += htbl->bits[_RV_insert_check(0,17,178,12,"jpeg_make_d_derived_tbl",l)];
      dtbl->maxcode[_RV_insert_check(0,18,179,7,"jpeg_make_d_derived_tbl",l)] = huffcode[_RV_insert_check(0,257,179,26,"jpeg_make_d_derived_tbl",p-1)]; /* maximum code of length l */
    } else {
      dtbl->maxcode[_RV_insert_check(0,18,181,7,"jpeg_make_d_derived_tbl",l)] = -1;	/* -1 if no codes of this length */
    }
  }
  dtbl->maxcode[_RV_insert_check(0,18,184,3,"jpeg_make_d_derived_tbl",17)] = 0xFFFFFL; /* ensures jpeg_huff_decode terminates */
  ...
  for (l = 1; l <= HUFF_LOOKAHEAD; l++) {
    for (i = 1; i <= (int) htbl->bits[_RV_insert_check(0,17,197,28,"jpeg_make_d_derived_tbl",l)]; i++, p++) {
      /* l = current code's length, p = its index in huffcode[] & huffval[]. */
      /* Generate left-justified code followed by all possible bit sequences */
      lookbits = huffcode[_RV_insert_check(0,257,200,18,"jpeg_make_d_derived_tbl",p)] << (HUFF_LOOKAHEAD-l);
      for (ctr = 1 << (HUFF_LOOKAHEAD-l); ctr > 0; ctr--) {
	dtbl->look_nbits[_RV_insert_check(0,256,202,2,"jpeg_make_d_derived_tbl",lookbits)] = l;
	dtbl->look_sym[_RV_insert_check(0,256,203,2,"jpeg_make_d_derived_tbl",lookbits)] = htbl->huffval[_RV_insert_check(0,256,203,29,"jpeg_make_d_derived_tbl",p)];
	lookbits++;
      }
    }
  }
		  ...
		  d_derived_tbl * htbl, int min_bits)
{
  ...
  while (code > htbl->maxcode[_RV_insert_check(0,18,338,17,"jpeg_huff_decode",l)]) {
    code <<= 1;
  ...
  return htbl->pub->huffval[ _RV_insert_check(0,256,356,10,"jpeg_huff_decode",htbl->valptr[_RV_insert_check(0,17,356,30,"jpeg_huff_decode",l)] +
			    ((int) (code - htbl->mincode[_RV_insert_check(0,17,357,23,"jpeg_huff_decode",l)])) )];
}
...
__boundcheck_metadata_store((void *)(&i),(void *)((size_t)(&i)+sizeof(i)*8-1));
  ...
  entropy = (huff_entropy_ptr)
    (*(void *(*)(j_common_ptr, int, size_t))(__boundcheck_ptr_reference(564,19,"jinit_huff_decoder",(void *)(cinfo->mem->alloc_small),(void *)cinfo->mem->alloc_small))) ((j_common_ptr) cinfo, JPOOL_IMAGE,
  ...
  for (i = 0; i < NUM_HUFF_TBLS; i++) {
    entropy->dc_derived_tbls[_RV_insert_check(0,4,572,5,"jinit_huff_decoder",i)] = entropy->ac_derived_tbls[_RV_insert_check(0,4,572,35,"jinit_huff_decoder",i)] = NULL;
  }
}
